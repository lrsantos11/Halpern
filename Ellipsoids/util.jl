...
function projection_intersection(x::AbstractVector, y::AbstractMatrix, Q::AbstractArray, k::AbstractVector, env::Gurobi.Env)
    max_vio = max_violation(x,y,Q,k)
    if max_vio <= 0.0
        return x
    end
    n = size(x, 1)
    ...
    @objective(model, Min, 0.5 * sum((y_var[j] - x[j])^2 for j in 1:n)) # 0.5 * ⟨y_var - x, y_var - x⟩
    for i in 1:m
        @expression(model, diff[j=1:n], y_var[j] - y[i,j])
        @expression(model, quad_expr, dot(diff, Q[i,:,:] * diff)) # ⟨y_var - y, Q*(y_var - y)⟩
        @constraint(model, quad_expr <= k[i]^2)
    end
    optimize!(model)
...
function all_elipsoid_projection(x::AbstractVector, y::AbstractVector, Q::AbstractMatrix, k::Real, env::Gurobi.Env)
